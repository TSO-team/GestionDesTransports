...
//INCLUSIONS
#include "../Inc/main.h"
#include "../Inc/piloteUSART2.h"
#include "../Inc/serviceBaseDeTemps.h"
#include "../Inc/serviceProtocole637.h"
...
void serviceProtocole637_attendUneRequete(void) {
    ...
    serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_TRANSMISSION] = serviceProtocole637_transmetLaLongueur;
    return;
}
...
void serviceProtocole637_transmetLaLongueur(void) {
    ...
    serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_TRANSMISSION] = serviceProtocole637_transmetUneDonnee;
}
...
void serviceProtocole637_transmetUneDonnee(void) {
    ...
    if (donnee == SERVICEPROTOCOLE637_DEBUT_DE_TRAME) {
        serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_TRANSMISSION] = serviceProtocole637_transmetUnZero;
        return;
    }
...
unsigned char serviceProtocole637_gereLaSynchronisationDesTransmissions(void) {
    ...
    if (serviceProtocole637_compteurDeSynchronisationDesTransmissions < SERVICEPROTOCOLE637_COMPTE_MAXIMAL_DE_SYNCHRONISATION_EN_TRANSMISSION)
...
void serviceProtocole637_attendUnDebut(void) {
    ...
    if (serviceProtocole637_reception == SERVICEPROTOCOLE637_DEBUT_DE_TRAME) {
        ...
        serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_RECEPTION] = serviceProtocole637_attendLaLongueur;
        return;
    }
...
void serviceProtocole637_attendLaLongueur(void) {
    if (serviceProtocole637_gereLaSynchronisationDesReceptions() == SERVICEPROTOCOLE637_RECEPTION_RETARDEE)
        return;
    if (serviceProtocole637_gereLeTempsDAttente() == SERVICEPROTOCOLE637_TEMPS_DEPASSE) {
        ...
        serviceProtocole637.information = INFORMATION_DISPONIBLE;
        serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_RECEPTION] = serviceProtocole637_attendUnDebut;
    }
    if (serviceProtocole637_gereUneReception() == SERVICEPROTOCOLE637_LECTURE_PAS_DISPONIBLE)
        return;
    serviceProtocole637_compteurDeTempsDAttente = 0;
    ...
    serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_RECEPTION] = serviceProtocole637_attendUneDonnee;
}
...
void serviceProtocole637_attendUneDonnee(void) {
    ...
    if (serviceProtocole637_gereUneReception() == SERVICEPROTOCOLE637_LECTURE_PAS_DISPONIBLE)
        return;
    serviceProtocole637_compteurDeTempsDAttente = 0;
    serviceProtocole637.octetsRecus[serviceProtocole637_compteurDeReceptions] = serviceProtocole637_reception;
    serviceProtocole637_checksum += serviceProtocole637_reception;
    serviceProtocole637_compteurDeReceptions++;
    if (serviceProtocole637_reception == SERVICEPROTOCOLE637_DEBUT_DE_TRAME) {
        serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_RECEPTION] = serviceProtocole637_attendUnZero;
        return;
    }
    if (serviceProtocole637_compteurDeReceptions == serviceProtocole637.nombreARecevoir) {
        serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_RECEPTION] = serviceProtocole637_attendUnChecksum;
        return;
    }
}
...
unsigned char serviceProtocole637_gereLaSynchronisationDesReceptions(void) {
    ...
    if (serviceProtocole637_compteurDeSynchronisationDesReceptions < SERVICEPROTOCOLE637_COMPTE_MAXIMAL_DE_SYNCHRONISATION_EN_RECEPTION)
...
unsigned char serviceProtocole637_gereLeTempsDAttente(void) {
    ...
    if (serviceProtocole637_compteurDeTempsDAttente < SERVICEPROTOCOLE637_COMPTE_MAXIMAL_ENTRE_LES_RECEPTIONS)
...
void serviceProtocole637_initialise(void) {
    ...
    serviceProtocole637.information = INFORMATION_TRAITEE;
    serviceProtocole637.statut = SERVICEPROTOCOLE637_PAS_D_ERREURS;
    serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_TRANSMISSION] = serviceProtocole637_attendUneRequete;
    serviceBaseDeTemps_execute[SERVICEPROTOCOLE637_PHASE_RECEPTION] = serviceProtocole637_attendUnDebut;
}
