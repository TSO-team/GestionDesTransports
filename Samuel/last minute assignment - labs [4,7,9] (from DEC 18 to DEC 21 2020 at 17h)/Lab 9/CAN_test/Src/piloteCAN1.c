...
unsigned char piloteCAN1_litUnMessageRecu(unsigned char *DonneesRecues) {
    if (HAL_CAN_GetRxMessage(&hcan1, CAN_FILTER_FIFO0, &piloteCAN1_reception, DonneesRecues) != HAL_OK)
...
unsigned int piloteCAN1_transmetDesDonnes(unsigned int Identification11Bits,
    ...
    if (HAL_CAN_AddTxMessage(&hcan1, &piloteCAN1_transmission, Donnees, &piloteCAN1_CasierPostal) != HAL_OK)
