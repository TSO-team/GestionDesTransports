...
#include "../Inc/interfaceB1.h"
#include "../Inc/processusBoutonConnecte.h"
...
unsigned char dataOUT[5] = {'A', 'l', 'l', 'o', '\0'};

void processusBoutonConnecte_attendApresLeBouton(void) {
    if (interfaceB1.etatDuModule == MODULE_PAS_EN_FONCTION)
        return;
    serviceBaseDeTemps_execute[PROCESSUSBOUTONCONNECTE_PHASE] = processusBoutonConnecte_attendUneDemande;
...
void processusBoutonConnecte_initialise(void) {
    serviceBaseDeTemps_execute[PROCESSUSBOUTONCONNECTE_PHASE] = processusBoutonConnecte_attendApresLeBouton;
}
