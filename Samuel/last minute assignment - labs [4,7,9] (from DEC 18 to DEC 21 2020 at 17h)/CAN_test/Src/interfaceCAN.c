...
{
  if (HAL_CAN_GetRxMessage(&hcan1, CAN_FILTER_FIFO0, &infoRX, Ptrdata) != HAL_OK) return 0;
...
unsigned int CAN_Send(unsigned int CallerID, unsigned char *Ptrdata, unsigned char len)
{
  ...
  infoTX.DLC = len;
  if (HAL_CAN_AddTxMessage(&hcan1, &infoTX, Ptrdata, &POBox) != HAL_OK) return 1;
  return 0;
}
...
unsigned int Init_interfaceCAN(void)
{
  ...
  if (HAL_CAN_ConfigFilter(&hcan1, &CANfilter) != HAL_OK) return 1;
